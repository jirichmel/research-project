...
def LASSO(POCET,LAMBDA_LENGTH,D,dE):
    ...
    for j in range(1,LAMBDA_LENGTH): ### BEZIME OD JEDNICKY KVULI ZAOKHROUHLOVANI NA HRANICI NULOVYCH KOEFICIENTU
        lasso = linear_model.Lasso(alpha=LAMBDA[j],fit_intercept=True, normalize=True, max_iter=5e04, warm_start=True, positive=False, copy_X=True, precompute=True, tol=1e-4)
        lasso.fit(D,dE)
        ...
        while(k<POCET):
            if len(coef)!=0 and np.max(coef)!=0:
                #Potencialni_values[k,j]=lasso.sparse_coef_.data[np.argmax(coef)]
...
def cross_validation_LASSO(cross_iter, POCET, LAMBDA_LENGTH, D, dE, test_size=7):
    ...
    for cv in range(cross_iter):
        ...
        D_CV, X_test, dE_CV, y_test = model_selection.train_test_split(D, dE, test_size=test_size, random_state=2*cv, shuffle = True)
...
def cross_validation_LASSO(cross_iter, POCET, LAMBDA_LENGTH, D, dE, test_size=7):
    ...
    for cv in range(cross_iter):
        ...
        cross_validation_descriptors[0, cv] = (ND[0])
...
D, DD, A1, A2, A3, B1, B2, B3, C3, D3, E3, F1, F2, F3, G = feature_space_generation(False, 1, 0.1, dimers, AB, *tiers[CISLO])
print("Tier", sum(tiers[CISLO])-1)
print(D.shape)
print(len(DD))

vysledky_cv = [cross_validation_LASSO(cross_iter, POCET, LAMBDA_LENGTH, D, dE)]
    
pickle.dump( vysledky_cv, open( "vysledky_cv_tier_" + str(sum(tiers[CISLO])-1)  + ".p", "wb" ) )

loaded = pickle.load( open( "vysledky_cv_tier_" + str(sum(tiers[CISLO])-1) + ".p", "rb" ) )

print("RMSE_CV: ", np.round(np.sum(loaded[0][0], axis = 1)/loaded[0][0].shape[1], 2), "eV.")
print("MaxAE_CV: ", np.round(np.sum(loaded[0][1], axis = 1)/loaded[0][1].shape[1], 2), "eV.")
print("Recovery %: ", np.round(np.sum(loaded[0][2], axis = 1)/loaded[0][2].shape[1], 2), "%.")
print()
