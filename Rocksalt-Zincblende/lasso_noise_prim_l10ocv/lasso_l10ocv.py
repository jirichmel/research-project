...
for _ in range(iterace): # do fifty drafts
    ...
    object_ids.append(cross_validation_LASSO_2D.remote(cross_iter, POCET, LAMBDA_LENGTH, feature_space_generation(False, feat, 1, dimers, AB, True, True, True, True, True, True)[0], dE, test_size=7))

sens_data = []
for i in range(iterace): # do fifty drafts
    sens_data.append(ray.get(object_ids[i]))###
pickle.dump( sens_data, open( "L10OCV" + ".p", "wb" ) )
