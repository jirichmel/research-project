...
@ray.remote
def tier_validation(POCET, LAMBDA_LENGTH, cross_iter, CISLO):
    ...
    vysledky_cv = [cross_validation_LASSO(cross_iter, POCET, LAMBDA_LENGTH, D, dE, test_size=7)]
...
for CISLO in [0,1,2,3,4,5]:
    object_ids.append(tier_validation.remote(POCET, LAMBDA_LENGTH, cross_iter, CISLO))

real = []
for CISLO in [0,1,2,3,4,5]:
    real.append(ray.get(object_ids[CISLO]))
