...
def cross_validation_LASSO_2D(cross_iter, POCET, LAMBDA_LENGTH, D, dE, test_size=7):
    ...
    for cv in range(cross_iter):
        ...
        RMSE_CV[cv] = np.sqrt(metrics.mean_squared_error(y_test, y_predicted))
        MaxAE_CV[cv] = metrics.max_error(y_test, y_predicted)

        # column indices of descriptors
        #desc = [819, (966, 2717), (966, 2717, 3110), (2151, 2717, 3110, 3399)]
        
        desc = (966, 2717)
        print(sorted(ND[0])==sorted(desc), sorted(ND[0]), sorted(desc))
        if sorted(ND[0])==sorted(desc):
            recovery[cv] = 1
...
# Tier 3
D, DD, A1, A2, A3, B1, B2, B3, C3, D3, E3, F1, F2, F3, G = feature_space_generation(True, False, 0, 1, dimers, AB, True, True, True, True, False, False)
for delta in DELTA:
    print("Shagging with ", delta)
    object_ids = []
    for j in range(num_selections):
        np.random.seed(j)
        print("Noising once...")
        noise = np.random.uniform(-delta, delta, dE.shape)
        dE_noised = dE + noise
        object_ids.append(cross_validation_LASSO_2D.remote(cross_iter, POCET, LAMBDA_LENGTH, D, dE_noised, test_size=7))    

    sens_data = []
    for i in range(len(object_ids)):
        sens_data.append([ray.get(object_ids[i])])
        
    pickle.dump( sens_data, open( "sens_data_E_L10OCV_Tier_3_delta_" + str(delta) + ".p", "wb" ) )


# Tier 4
D, DD, A1, A2, A3, B1, B2, B3, C3, D3, E3, F1, F2, F3, G = feature_space_generation(True, False, 0, 1, dimers, AB, True, True, True, True, True, False)
for delta in DELTA:
    print("Shagging with ", delta)
    object_ids = []
    for j in range(num_selections):
    ...
    pickle.dump( sens_data, open( "sens_data_E_L10OCV_Tier_4_delta_" + str(delta) + ".p", "wb" ) )
