...
#matrix = ngram.gen_ngram_matrix(label, df_frac_train, df_latt_train, df_gen_train, 3, ngram.ALL_COORDINATIONS_TRIGRAM)
#print("Dimensions: ", matrix.shape)
#np.savetxt("trigram" + "_" + label + "_competition_train.csv", matrix, delimiter=",")

#y = df_egy_train[["formation_energy_ev_natom", "bandgap_energy_ev"]].to_numpy()

#matrix = ngram.gen_ngram_matrix(label, df_frac_test, df_latt_test, df_gen_test, 3, ngram.ALL_COORDINATIONS_TRIGRAM)
#print("Dimensions: ", matrix.shape)
#np.savetxt("trigram" + "_" + label + "_competition_test.csv", matrix, delimiter=",")
...
def calc_parts(name, NUM_CPUS, type_of_calc, df_frac, df_latt, df_gen, n, coordinations):
    chunks = np.array_split(df_gen, NUM_CPUS)
    object_ids = []
    decorated_func = ray.remote(ngram.gen_ngram_matrix)
    # Parallel forcycle:
    for chunk in chunks: # get the object ids into a list
        ...
        object_ids.append(decorated_func.remote(type_of_calc, df_frac, df_latt, chunk, n, coordinations))
    actual_parts = []
...
calc_parts("unigram" + "_" + label + "_competition_train.csv", NUM_CPUS, label, df_frac_train, df_latt_train, df_gen_train, 1, ngram.ALL_COORDINATIONS_UNIGRAM)
calc_parts("unigram" + "_" + label + "_competition_test.csv", NUM_CPUS, label, df_frac_test, df_latt_test, df_gen_test, 1, ngram.ALL_COORDINATIONS_UNIGRAM)

calc_parts("bigram" + "_" + label + "_competition_train.csv", NUM_CPUS, label, df_frac_train, df_latt_train, df_gen_train, 2, ngram.ALL_COORDINATIONS_BIGRAM)
calc_parts("bigram" + "_" + label + "_competition_test.csv", NUM_CPUS, label, df_frac_test, df_latt_test, df_gen_test, 2, ngram.ALL_COORDINATIONS_BIGRAM)

calc_parts("trigram" + "_" + label + "_competition_train.csv", NUM_CPUS, label, df_frac_train, df_latt_train, df_gen_train, 3, ngram.ALL_COORDINATIONS_TRIGRAM)
calc_parts("trigram" + "_" + label + "_competition_test.csv", NUM_CPUS, label, df_frac_test, df_latt_test, df_gen_test, 3, ngram.ALL_COORDINATIONS_TRIGRAM)
