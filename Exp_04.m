...
Distsort=sort(Dist(1:AtomNMetal,AtomNMetal+1:AtomN),2);
%Distsort=sort(Dist);
...
for u = 1:AtomNMetal
    ...
    oxindx=find(Dist(u,:)<=bound & Dist(u,:)>0);
...
sqrt(norm(Energy_guess-TestEnergy(:,3),2)^2/size(TestEnergy,1)) %0.0714
