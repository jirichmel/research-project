...
count = 1; datacount = 1;
Data=zeros(size(TestLattice,1)+size(TrainLattice,1),21*3+4);
...
sqrt(norm(Energy_guess-TestEnergy(:,3),2)^2/size(TestEnergy,1)) %0.0445
...
sqrt(norm(Bond_guess-TestEnergy(:,4),2)^2/size(TestEnergy,1)) %0.3258
